...
def plot_freq_res(delta_t = 0.001, DM=1000, central_freq_mhz = 150):
    T =  [i * delta_t for i in range(1, 50)]
    BW = [compute_required_bandwidth_khz(central_freq_mhz, DM, t) for t in T]
    plt.plot(T, BW)
    plt.title("Required frequency bandwidth as a function of integration time.")
...
def plot_freq_as_dm(central_freq_mhz = 150):
    ...
    plt.title("Required frequency bandwidth as a function of DM.")
