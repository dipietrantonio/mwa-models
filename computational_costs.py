...
from models import MWA_PHASE_1 as MWA_MODEL
from plotting import make_barplot
from basics import npixels
...
def plot_imaging_costs_frb_case(ax):
    integration_time = 0.05 # s
    longest_baseline = 300 # m
    Np = npixels(FOV, FREQ, longest_baseline)
...
def plot_imaging_costs_as_function_of_int_time():
    ...
    gcosts = []
    fcosts = []
    Np = npixels(MWA_MODEL.FoV, FREQ, 300)
...
def plot_beamforming_vs_imaging_frb_case(ax):
    ...
    title = "Computational cost of beamforming and imaging\nfor MWA Phase II Compact\n(frequency = 150MHz, integration time = 50ms)"
    ax.set_title(title)
...
if __name__ == "__main__":
    ...
    plot_beamforming_vs_imaging_frb_case(ax1)
    plt.show()
