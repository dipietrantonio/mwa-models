...
def dispersive_delay_s(DM, f1_ghz, f2_ghz):
    return dispersive_delay_ms(DM, f1_ghz, f2_ghz) / 1000
...
def pixsize_deg(longest_baseline_m, freq_hz, oversampling_factor = 2):
    ...
    pixsize_rad = 1 / (oversampling_factor * u_max)
    return  pixsize_rad * (180 / pi)
    

def npixels(FoV_deg2, freq_hz, longest_baseline_m, oversampling_factor = 2):
    resolution = pixsize_deg(longest_baseline_m, freq_hz, oversampling_factor)
    return (FoV_deg2 / resolution**2)


def expected_frb_daily_rate(ref_rate, ref_freq_hz, ref_fluence, freq_hz, fluence, alpha):
